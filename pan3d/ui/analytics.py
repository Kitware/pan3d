...
from trame.widgets import html, vuetify3 as v3, plotly
...
class ChartPanel(v3.VCard):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(rounded=True, classes="controllerbr w-50", **kwargs)
        self.source = source
        with self:
            with v3.VCardText(
                v_show=(toggle, True),
            ...
            with v3.VCardTitle(
                classes=(
                    f"`d-flex pa-1 bg-white ${{ {toggle} ? 'controller-content rounded-b border-t-thin':'rounded-circle'}}`",
                ),
                style="z-index: 1;",
            ):
                v3.VBtn(
                ...
                v3.VSelect(
                    v_if=toggle,
                    label="Plot Type",
                    v_model=("active_plot", "Spatial"),
                    items=("plots", ["Spatial", "Temporal"]),
                    hide_details=True,
                    dense=True,
                    variant="outlined",
                    density="compact",
